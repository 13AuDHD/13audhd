/*
  Photography archive data

  Add each new photograph to img/photos/, then add one object below.

Required:
  src, thumb, alt, title, year, tags

Recommended:
  date

Optional:
  description

Use date in YYYY-MM-DD format for accurate
newest-first and oldest-first sorting.

When date is omitted, the archive sorts the
photograph using January 1 of its year.

  Optional:
    description

  The thumbnail can point to the full image while the archive is small.
  For a large archive, create smaller thumbnail files in:
    img/photos/thumbs/
*/

window.PHOTO_LIBRARY = [
  {
    src: "img/photos/photo-001.jpeg",
    thumb: "img/photos/thumbs/photo-001.jpeg",
    alt: "A mountain road beneath gathering clouds",
    title: "Weather Moving In",
    date: "2025-04-24",
    year: 2025,
    tags: ["Mountains", "Roads", "Weather"],
    description: "A quiet road in the San Juan Mountains just before the weather changed."
  },
  {
    src: "img/photos/photo-002.jpeg",
    thumb: "img/photos/thumbs/photo-002.jpeg",
    alt: "Horizon parked beside a forest road",
    title: "Camp for the Night",
    date: "2026-05-21",
    year: 2026,
    tags: ["Horizon", "Camping", "Forest"],
    description: "A temporary home among the trees."
  },
  {
    src: "img/photos/photo-003.jpeg",
    thumb: "img/photos/thumbs/photo-003.jpeg",
    alt: "Morning light crossing a mountain ridge",
    title: "First Light",
    date: "2026-02-24",
    year: 2026,
    tags: ["Mountains", "Sunrise", "Colorado"],
    description: "Early light reaching across the western slope."
  },
  {
    src: "img/photos/photo-004.jpeg",
    thumb: "img/photos/thumbs/photo-004.jpeg",
    alt: "A fox standing near the edge of a road",
    title: "Scout",
    date: "2024-08-23",
    year: 2024,
    tags: ["Wildlife", "Fox", "Roads"],
    description: "The campsite encounter that started a company."
  },
  {
    src: "img/photos/photo-005.jpeg",
    thumb: "img/photos/thumbs/photo-005.jpeg",
    alt: "Rain falling across distant mountains",
    title: "Across the Valley",
    date: "2024-05-13",
    year: 2024,
    tags: ["Weather", "Mountains", "Rain"],
    description: "A curtain of rain moving through the distance."
  },
  {
    src: "img/photos/photo-006.jpeg",
    thumb: "img/photos/thumbs/photo-006.jpeg",
    alt: "A narrow dirt road through evergreen trees",
    title: "The Long Way",
    date: "2025-06-04",
    year: 2025,
    tags: ["Roads", "Forest", "Colorado"],
    description: "The slower route usually has the better view."
  },
  {
    src: "img/photos/photo-007.jpeg",
    thumb: "img/photos/thumbs/photo-007.jpeg",
    alt: "A camp chair overlooking a wide landscape",
    title: "No Urgent Destination",
    date: "2024-05-16",
    year: 2024,
    tags: ["Camping", "Landscape", "Quiet"],
    description: "A place to stop without needing a reason."
  },
  {
    src: "img/photos/photo-008.jpeg",
    thumb: "img/photos/thumbs/photo-008.jpeg",
    alt: "Snow resting on a dark mountain forest",
    title: "Winter Line",
    date: "2025-12-13",
    year: 2025,
    tags: ["Winter", "Forest", "Mountains"],
    description: "Snow separating the ridgeline from the trees."
  },
  {
    src: "img/photos/photo-009.jpeg",
    thumb: "img/photos/thumbs/photo-009.jpeg",
    alt: "Clouds glowing above the desert at sunset",
    title: "After the Heat",
    date: "2024-07-30",
    year: 2024,
    tags: ["Desert", "Sunset", "Weather"],
    description: "The sky changing color after the ground began to cool."
  },
  {
    src: "img/photos/photo-010.jpeg",
    thumb: "img/photos/thumbs/photo-010.jpeg",
    alt: "Horizon traveling along a rocky trail",
    title: "Beyond Pavement",
    date: "2025-10-14",
    year: 2025,
    tags: ["Horizon", "Roads", "Overland"],
    description: "A road rough enough to make the destination feel earned."
  },
  {
    src: "img/photos/photo-011.jpeg",
    thumb: "img/photos/thumbs/photo-011.jpeg",
    alt: "A small flower growing beside a trail",
    title: "Trailside",
    date: "2026-05-19",
    year: 2026,
    tags: ["Plants", "Close-up", "Nature"],
    description: "Something small enough to miss while moving too quickly."
  },
  {
    src: "img/photos/photo-012.jpeg",
    thumb: "img/photos/thumbs/photo-012.jpeg",
    alt: "Stars visible above a dark campsite",
    title: "Camp After Dark",
    date: "2023-03-04",
    year: 2023,
    tags: ["Night", "Camping", "Stars"],
    description: "The view after every other light disappeared."
  },
  {
    src: "img/photos/photo-013.jpeg",
    thumb: "img/photos/thumbs/photo-013.jpeg",
    alt: "A campsite next to the river",
    title: "Riverside Camping",
    date: "2026-07-04",
    year: 2026,
    tags: ["River", "Camping", "Horizon"],
    description: "Horizon parked alongside a peaceful river."
  },
  {
    src: "img/photos/photo-014.jpeg",
    thumb: "img/photos/thumbs/photo-014.jpeg",
    alt: "Wolf mural on the side of a quiet building",
    title: "Wolf Mural",
    date: "2024-10-27",
    year: 2024,
    tags: ["Mural", "City", "Sunset"],
    description: "A wolf mural on the side of a building."
  },
    {
    src: "img/photos/photo-015.jpeg",
    thumb: "img/photos/thumbs/photo-015.jpeg",
    alt: "Looking off a bridge down at a river",
    title: "The Piedra River",
    date: "2026-06-03",
    year: 2026,
    tags: ["Forest", "Nature", "River"],
    description: "Looking off an old bridge at the Piedra River."
  }
];
